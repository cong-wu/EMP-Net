...
def train_epoch(
    ...
    for cur_iter, task_dict in enumerate(train_loader):
        ...
        patch_target_logits = model_dict['patch_logits']
        frame_target_logits = model_dict['frame_logits']
        token_target_logits = model_dict['token_logits']
        patch_class_logits = model_dict['patch_class_logits'][-patch_target_logits.shape[0]:]
        frame_class_logits = model_dict['frame_class_logits'][-patch_target_logits.shape[0]:]
        token_class_logits = model_dict['token_class_logits'][-patch_target_logits.shape[0]:]

        target_logits1 = patch_target_logits + frame_target_logits + token_target_logits
        target_logits2 = patch_class_logits + frame_class_logits + token_class_logits
        target_logits = (target_logits1 + target_logits2) / 6.0
        ...
        if hasattr(cfg.TRAIN,"USE_CLASSIFICATION") and cfg.TRAIN.USE_CLASSIFICATION:
            if hasattr(cfg.TRAIN,"USE_CLASSIFICATION_ONLY") and cfg.TRAIN.USE_CLASSIFICATION_ONLY:
                ...
                if hasattr(cfg.TRAIN,"USE_CONTRASTIVE") and cfg.TRAIN.USE_CONTRASTIVE:
                    ...
                    loss1 = F.cross_entropy(model_dict["patch_logits"], task_dict["target_labels"].long()) +  \
                            F.cross_entropy(model_dict["frame_logits"], task_dict["target_labels"].long()) + \
                            F.cross_entropy(model_dict["token_logits"], task_dict["target_labels"].long())

                    labels = torch.cat([task_dict["support_labels"], task_dict["target_labels"]], 0).long()
                    loss2 = F.cross_entropy(model_dict["patch_class_logits"], labels) + \
                            F.cross_entropy(model_dict["frame_class_logits"], labels) + \
                            F.cross_entropy(model_dict["token_class_logits"], labels)
                    loss = loss1 + loss2
