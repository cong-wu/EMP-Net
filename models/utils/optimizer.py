...
def construct_optimizer(model, cfg):
    ...
    if cfg.TRAIN.ONLY_LINEAR:
        ...
        assert len(list(filter(lambda p: p.requires_grad, model.parameters()))) == len(non_bn_parameters) + \
            len(bn_params) + \
            len(head_parameters) + \
            len(no_weight_decay_parameters) + \
            num_skipped_param, "parameter size does not match: {} + {} != {}".format(len(non_bn_parameters), len(bn_params), len(list(filter(lambda p: p.requires_grad, model.parameters()))))
